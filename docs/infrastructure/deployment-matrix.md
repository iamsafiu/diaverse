...
| `diaverse-content` production | `diaverse-content` | overseas content server behind `diaverse-prod` edge proxy | `/srv/diaverse-content` | Docker Compose project `diaverse-content` | `https://diaverse.app/ru/learn/*`, `https://diaverse.app/_diaverse-content/_next/*` | HTTPS upstream to content server; no browser cookies forwarded | [content factory](../runbooks/content-factory-foreign-server.md) |
| `diaverse-content` development | `diaverse-content` | overseas/staging content server behind `diaverse-dev` edge proxy | `/srv/diaverse-content-dev` | Docker Compose project `diaverse-content-dev` | `https://dev.diaverse.app/ru/learn/*`, `https://dev.diaverse.app/_diaverse-content/_next/*` | HTTPS upstream to content server; no browser cookies forwarded | [content factory](../runbooks/content-factory-foreign-server.md) |
...
| `aibot` ops-agent bot | `aibot` | `diaverse-bots` | `/srv/aibot` | Docker Compose service `copywriting-ops-agent` | none | Telegram polling, signed Diaverse ops API, local Codex runner | [ops-agent](../runbooks/ops-agent.md) |
