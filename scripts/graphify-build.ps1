...
try {
    Write-Host "DEBUG [graphify] Starting workspace graph build with HTML cap override"
    & $pythonExe $helperScript --workspace-root $workspaceRoot --mode build
    if ($LASTEXITCODE -ne 0) {
        throw "Graphify initial build failed with exit code $LASTEXITCODE"
    }
} finally {
    Pop-Location
}
